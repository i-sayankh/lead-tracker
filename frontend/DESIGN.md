...
## Lead Tracker adaptation

The Linear system above describes a dark marketing site. Lead Tracker is a dense internal data tool, so it keeps Linear's tokens, type scale, radius, spacing and restraint, and adds the few things the source leaves out. These rules win over the source where they differ. All tokens live as CSS variables in `src/index.css`.

### Themes

- **Dark** (when `prefers-color-scheme: dark`): the Linear palette as specified: canvas `#010102`, surfaces `#0f1011`/`#141516`, hairline `#23252a`, ink `#f7f8f8`.
- **Light** (default): the source's inverse tokens extended into a full light ladder: canvas `#ffffff`, surface-1 `#f7f8f8`, surface-2 `#f0f1f2`, hairline `#e6e7e9`, ink `#15161a`, ink-subtle `#62666d`. The source says "don't ship a light-mode marketing page"; this is not a marketing page, and a CRM tool used all day needs a light theme.
- Primary accent stays lavender `#5e6ad2` in both themes; used only for the primary button, focus rings and links. Text on it is white (4.7:1).
- In the light theme the primary hover darkens to `#4f5bc4` (the source lightens it to `#828fff`, which fails contrast with white text on a light page).

### Typography

- Family: `SF Pro Display, -apple-system, system-ui, "Segoe UI", Roboto, sans-serif` (the source's documented fallback). No web font download; Inter is not used.
- Mono (`ui-monospace, "SF Mono", Menlo, Consolas, monospace`) for phone numbers and keyboard hints only.
- App sizes: page title `headline` 28/600 (-0.6px), section and dialog titles `card-title` 22/500, table and form text `body-sm` 14/400, meta and table headers `caption` 12/400–500.

### Status pills (Linear product tag palette, muted)

Each status has a text label **and** a color; color is never the only signal.

| Status | Light bg / text | Dark bg / text |
|---|---|---|
| `new` | `#e8eafb` / `#3f48a8` | `#1f2245` / `#aab1ff` |
| `contacted` | `#fbf3db` / `#7a5200` | `#3a2e0f` / `#f2c96b` |
| `qualified` | `#e6f4ea` / `#1d6b35` | `#12301c` / `#6fd08c` |
| `lost` | `#f1f1f2` / `#55585e` | `#232428` / `#a4a8b0` |

All pairs meet 4.5:1 contrast.

### Semantic

- **Danger** `#c5362f` light / `#ff7b72` dark, for inline field errors and error toasts (the source has no error styling).
- **Success** stays `#27a644` (source) for the success toast marker.
...
- **Inputs**: `text-input` from the source, min height 40px; focus = 2px lavender outline.
- **Toasts**: bottom-right stack, surface-2, hairline border, auto-dismiss after 4s.

### Motion

Only 120–150ms color/opacity transitions on hover and focus, and the skeleton shimmer. All of it is disabled under `prefers-reduced-motion: reduce`. No scroll-entry animations, gradients or decorative imagery.
